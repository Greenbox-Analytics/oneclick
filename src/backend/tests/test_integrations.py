...
class TestIntegrationGated:
    """SP3: Free users cannot start OAuth for Slack/Notion/Monday; Drive is ungated."""
    ...
    def test_monday_oauth_start_free_returns_402(self, client, monkeypatch):
        """Free user -> GET /integrations/monday/auth returns 402."""
        from subscriptions import enforcement

        monkeypatch.setattr(enforcement, "_service", lambda: self._denied_service("monday"))

        resp = client.get("/integrations/monday/auth")
        assert resp.status_code == 402
        detail = resp.json()["detail"].lower()
        assert "monday" in detail or "integration" in detail or "pro" in detail
