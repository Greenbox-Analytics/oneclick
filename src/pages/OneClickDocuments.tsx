...
const OneClickDocuments = () => {
  ...
  const fetchProjectFilesForValidation = async (projectId: string): Promise<ArtistFile[]> => {
    if (projectFilesById[projectId]) return projectFilesById[projectId];
    const response = await fetch(`${API_URL}/files/${projectId}?user_id=${user?.id}`);
    if (!response.ok) throw new Error("Failed to load existing files for duplicate check");
  ...
  useEffect(() => {
    if (artistId && user) {
      ...
      fetch(`${API_URL}/artists?user_id=${user.id}`)
        .then(res => res.json())
        .then((data: Artist[]) => { const artist = data.find(a => a.id === artistId); if (artist) setArtistName(artist.name); setIsLoadingArtist(false); })
        .catch(err => { console.error("Error fetching artist:", err); setIsLoadingArtist(false); });
    }
  }, [artistId, user]);
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/registry/works/${selectedWorkId}/files?user_id=${user.id}`)
      .then((res) => res.json())
      .then((data) => { setWorkFiles(data.files || []); setLoadingWorkFiles(false); })
      .catch((err) => { console.error("Error fetching work files:", err); setWorkFiles([]); setLoadingWorkFiles(false); });
  }, [selectedWorkId, user]);
    ...
    try {
        ...
        if (contractFiles.length > 0) {
            ...
            for (const file of contractFiles) {
                ...
                const uploadRes = await fetch(`${API_URL}/contracts/upload`, { method: "POST", body: formData });
                if (!uploadRes.ok) {
        ...
        if (royaltyStatementFile) {
            ...
            if (targetProjectId) {
              ...
              const projectFileNames = new Set(projectFiles.map(existing => normalizeFileName(existing.file_name)));
              if (projectFileNames.has(normalizeFileName(royaltyStatementFile.name))) {
                showPersistentDuplicateToast(`A file named "${royaltyStatementFile.name}" already exists in this project.`);
        ...
        const queryParams = new URLSearchParams({
            user_id: user.id, project_id: finalProjectId, royalty_statement_file_id: finalStatementId
        });
  ...
  const handleConfirmResultsWithContext = async () => {
      ...
      try {
          const response = await fetch(`${API_URL}/oneclick/confirm`, {
              method: "POST", headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                  contract_ids: lastCalculationContext.contractIds, royalty_statement_id: lastCalculationContext.statementId,
                  project_id: lastCalculationContext.projectId, user_id: user.id, results: calculationResult
              })
          });
