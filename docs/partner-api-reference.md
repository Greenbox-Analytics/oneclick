...
| `429` | `rate_limited` | The key sent more than 60 requests in a minute. Response carries `Retry-After: 60`. The limit is per key — split heavy batches across keys, or ask us to raise it |
...
- **Each key is capped at 60 requests per minute.** Over that the call is a `429` with `Retry-After: 60`, before any work or billing. The cap is per KEY, so parallel workloads are cleanest on separate keys — a team admin can mint them in Msanii. Ask us if a workload genuinely needs a higher ceiling.
