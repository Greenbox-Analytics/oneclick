...
task dev             # Frontend :8080 + backend :8000 + partner API :8001
task dev:partner     # Partner API alone — sets PARTNER_API_ENABLED for that process only
...
| `/oneclick/v1` `/registry/v1` `/splitsheet/v1` `/zoe/v1` | Partner API | External partner, tool-first paths, one router per tool (2026-09-05; `/partner/v1` is gone — each router carries its own prefix, `main.py` mounts them bare, and the host lockdown allowlists exactly those routes by regex, never by prefix, since the prefixes are shared with product routes. `/me` and `/test` were removed 2026-09-06: no account-level routes, the free key check is `GET /zoe/v1/models`): ONE key type (org credential, hash-only, human-minted in `/teams` or by a Msanii admin — the bearer surface never mints; `20260904000002` dropped the old backend/license hierarchy), stateless royalty calculation at `POST /oneclick/v1/royalties`, **splits** at `POST /registry/v1/splits` (renamed from `/contract-terms` 2026-09-06) (`partner_api/registry.py`: PDFs in, SSE out — `contract_terms` in the royalties input shape via `models.from_contract_data` + the Registry `splits` pivot; priced by `partner_registry_parse`, 30), **split sheets** at `POST /splitsheet/v1/documents` (`partner_api/splitsheet.py`: JSON in, the PDF/DOCX out through the product generators; no LLM so always the base, per document; `partner_split_sheet`, 20 — both rows seeded by `20260906000001`), and **Zoe, OpenAI-compatible and stateless** at `POST /zoe/v1/chat/completions` + `GET /zoe/v1/models` (`partner_api/zoe.py`: OpenAI SDK works with `base_url=<host>/zoe/v1`, `model="zoe"`; no stored docs, no memory; priced by `credit_prices.partner_zoe_message`, `20260905000001`, seeded 5; charge-on-delivery in both modes; every completion pays the base — no free fast path, no idempotency). **Every response reports its charge** (2026-09-06): compute `compute_charge` BEFORE the frame, debit AFTER it. `billing: {credits, request_id[, replayed]}` in result events / Zoe bodies / the stream's stop frame, `Msanii-*` headers on split sheets (CORS `expose_headers`), `{credits: 0}` on errors; `replayed` from `partner_api.service.already_charged` (advisory, Idempotency-Key only). Results are sectioned: royalties `summary` + `payments[{song, payee, share, amounts}]`, splits `contract_terms` + `splits{main_artist, parties}`. No token counts leave the API. Gated per org by `organizations.partner_api_enabled`, ON by default for enterprise orgs (`create_enterprise_org` / `set_org_kind`→enterprise both write it; `20260904000001` backfills), Msanii-admin toggle for self-serve (see docs/superpowers/specs/2026-08-10-oneclick-partner-api-design.md). Msanii-admin key endpoints `/admin/orgs/{id}/partner-keys`. Partner-facing reference: `docs/partner-api-reference.md`; drop-in client + smoke test: `examples/partner-api/msanii_partner.py` (wire format pinned by `tests/test_partner_example_client.py`). Keys can be grouped into `partner_key_folders` (spend attributed by the key's current folder, `byFolder` in the usage payload); inactive keys are hidden after 30 days (`revoked_at`, `partner_api.service.is_hidden`), never deleted; Msanii admins read any org's usage at `GET /admin/orgs/{id}/usage` (migration `20260907000001`, unapplied). **Abuse controls all hang off the ONE auth chokepoint, `router.get_partner_context`** (migration `20260907000002`, applied), so a route added later inherits them or none: a per-KEY rolling 60s rate limit (`PARTNER_RATE_LIMIT_PER_MIN`, default 60, `0` disables) returning 429 + `Retry-After`, and one `partner_api_requests` row per auth attempt — the limiter's counter AND the only record of source IP / 401s (nothing else logs a failed auth). The IP is the LAST `X-Forwarded-For` hop, since Cloud Run appends the real client and everything before it is caller-forged. Both the limiter and the log FAIL OPEN — the pool balance is still the hard ceiling. `key_prefix` is recorded on a 401 only when it starts with `mk_live_`, so a foreign secret pasted into the header leaves no fragment. The table is a rolling window purged by the daily sweep (`PARTNER_LOG_RETENTION_DAYS`, 30); per-key SPEND stays on `credit_ledger.metadata.partner_key_id` forever. Incident path: `POST /admin/partner-keys/lookup` (POST so a pasted key never reaches a URL; prefix-only, returns a LIST + org + recent IPs) -> revoke in place, via the key's OWN `org_id` and NEVER the drawer's selection, behind a team-naming confirm and only on a still-active key (re-revoking would reset the 30-day hide clock). The drawer's per-org key table stays read-only. |
...
The partner API has its own rows — **`partner_oneclick_run` 30, `partner_zoe_message` 5, `partner_registry_parse` 30, `partner_split_sheet` 20** — pricing each partner deliverable independently of the product's. **`ai_pricing.compute_charge()` is the ONE charge formula** (base / metered / base + size tail) — `debit_for_action` and every partner endpoint call it; another `max()` anywhere is a bug. The API service (`PARTNER_API_ENABLED`) also 404s every path outside the partner routers' own routes and `/health` via `_partner_host_lockdown` in `main.py`.
...
| `partner_key_created` / `partner_key_revoked` | `partner_api/org_router.py` | Org-admin key console (`via: "portal"`). `distinct_id` = the acting admin. Machine minting under a root key stays untracked — it is the partner's traffic, not a Msanii user's action. |
| `partner_oneclick_calc_started` / `_completed` / `_failed` | `partner_api/router.py` | Partner calculate funnel. `distinct_id` = the ORG id, not a user — a machine call has no user. Deliberately NOT `tool_used`: partner runs bill a different price row and must not blend into product tool counts. |
| `partner_zoe_started` / `_completed` / `_failed` | `partner_api/zoe.py` | Partner Zoe funnel, same rules (`distinct_id` = org id, not `tool_used`). `stream` property splits the two modes. |
| `partner_registry_parse_started` / `_completed` / `_failed`, `partner_splitsheet_started` / `_completed` / `_failed` | `partner_api/registry.py`, `partner_api/splitsheet.py` | Splits and split-sheet funnels, same rules (`distinct_id` = org id, not `tool_used`). |
...
| **Dev API** | — | Cloud Run (`msanii-api-dev`, `deploy-api-dev.yml`, `PARTNER_API_ENABLED=true`) | Push to `main` (when `src/backend/**` changes) |
| **Prod API** | — | Cloud Run (`msanii-api`, `deploy-api.yml`) | Published tag release (`v*`) |

Both API services sit behind a **global external ALB** (`scripts/setup-api-load-balancer.sh` — serverless NEG + Certificate Manager DNS-authorized cert, because a domain mapping isn't offered in `northamerica-northeast2` and a classic managed cert can't renew behind a proxy). Setting the `DEV_API_HOSTNAME` / `PROD_API_HOSTNAME` GitHub secret flips that workflow to `--ingress internal-and-cloud-load-balancing` and health-checks the public hostname instead of `.run.app`; unset, it deploys exactly as before, so merging can never strand a service behind an LB that isn't built yet. `--allow-unauthenticated` is still REQUIRED under LB-only ingress — ingress is the network gate, IAM is a separate one, and a serverless NEG doesn't authenticate as a principal, so the public invoker binding is needed regardless of region or LB. Under domain restricted sharing that binding is refused: **`constraints/iam.allowedPolicyMemberDomains` is a LEGACY MANAGED constraint accepting Cloud Identity customer IDs ONLY** — there is no public value and `principalSet://goog/public:all` is rejected by the API, so any "add the public principal alongside your customer ID" advice is wrong. The supported escape is a TAG-CONDITIONAL policy, also the narrowest: a project-level policy (`inheritFromParent: true`, one rule `allowAll: true` conditioned on `resource.matchTag("msanii-484501/allUsersIngress", "True")`) applied 2026-09-08, then per service tag + grant via `scripts/allow-public-invoker.sh`. Untagged resources keep full DRS. `msanii-api` (prod) still needs its per-service half. **If an LB is put in front, `partner_api.service.client_ip` needs re-checking**: it takes the LAST `X-Forwarded-For` hop, and an ALB adds one, so the real client moves to second-to-last — verify against a known IP before trusting the log.
